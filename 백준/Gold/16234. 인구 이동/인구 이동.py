...
def bfs(si, sj):
    ...
    # 초기 상태 설정
    q.append((si, sj))      # 큐에 시작 좌표 삽입
    v[si][sj] = 1           # 방문 표시
    alst = [(si, sj)]       # 연합에 포함된 나라 리스트
    sm = arr[si][sj]        # 연합의 인구 총합

    # 네 방향 이동을 나타내는 리스트
    dx = [-1, 0, 1, 0]  # 상, 하, 좌, 우 방향
    dy = [0, 1, 0, -1]  # 상, 하, 좌, 우 방향
    ...
    while q:
        ...
        for i in range(4):
            ...
            if 0 <= nx < N and 0 <= ny < N and v[nx][ny] == 0 and L <= abs(arr[x][y] - arr[nx][ny]) <= R:
                ...
                sm += arr[nx][ny]     # 인구 총합에 더함
    
    # 연합이 형성된 경우 (2개 이상의 나라로 구성된 경우)
    if len(alst) > 1:
        for ti, tj in alst:
            arr[ti][tj] = sm // len(alst)  # 연합에 속한 나라의 인구를 평균값으로 설정
...
while True:
    ...
    flag = 0  # 연합이 발생했는지 여부를 저장

    # 모든 나라를 탐색하며 연합 형성
    for i in range(N):
        for j in range(N):
            if v[i][j] == 0:  # 아직 방문하지 않은 나라에 대해
                flag = max(flag, bfs(i, j))  # BFS 탐색을 수행하고 연합이 있었다면 1 반환
    
    # 연합이 발생하지 않았으면 종료
    if flag == 0:
        break
    ans += 1  # 연합이 발생한 경우 인구 이동 횟수 증가
